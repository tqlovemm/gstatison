...
@implementation XDWSeekRecordCell
...
+ (instancetype)cellWithTableView:(UITableView *)tableView {
    // 1、创建Cell
    static NSString *identifier = @"XDWSeekRecordCellID";
    XDWSeekRecordCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[XDWSeekRecordCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:identifier];
    }
    // 3、返回Cell
    return cell;
}
...
- (void)setRecordFrame:(XDSeekRecordFrame *)recordFrame {
    ...
    if (record.status == 10) { // 觅约等待中
        ...
        [self.costStateLabel setTextColor:[UIColor orangeColor]];
    } else if (record.status == 11) { // 觅约成功
