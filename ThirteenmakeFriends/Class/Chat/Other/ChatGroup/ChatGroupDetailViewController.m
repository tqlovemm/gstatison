...
//        [_clearButton setBackgroundColor:[UIColor colorWithRed:87 / 255.0 green:186 / 255.0 blue:205 / 255.0 alpha:1.0]];
        [_clearButton setBackgroundColor:RGB(29, 185, 14)];
...
//        [_dissolveButton setBackgroundColor: [UIColor colorWithRed:191 / 255.0 green:48 / 255.0 blue:49 / 255.0 alpha:1.0]];
        [_dissolveButton setBackgroundColor:RGB(246, 72, 68)];
...
- (UIButton *)exitButton
{
    if (_exitButton == nil) {
        ...
        [_exitButton setBackgroundColor:RGB(246, 72, 68)];
...
//    int row = (int)([self.dataSource count] + 1) / kColOfRow;
    int tmp,row = 0;
    if (self.occupantType == GroupOccupantTypeOwner) { // 解决非群主查看群详情高度不对的问题
        tmp = ([self.dataSource count] + 1) % kColOfRow; // 行号
        row = (int)([self.dataSource count] + 1) / kColOfRow; // 行数
    } else {
        tmp = [self.dataSource count] % kColOfRow; // 行号
        row = (int)[self.dataSource count] / kColOfRow; // 行数
    }
    row += tmp == 0 ? 0 : 1;
