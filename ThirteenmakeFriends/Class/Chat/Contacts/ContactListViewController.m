...
//    return [[UserProfileManager sharedInstance] getNickNameWithUsername:self.username];
    // hx头像昵称
    return [UserProfileManager getNickById:self.username];
}
...
- (void)viewDidLoad {
    ...
    [right addTarget:self action:@selector(addContactAction) forControlEvents:UIControlEventTouchUpInside];
    [right setImage:[UIImage imageNamed:@"add_friend"] forState:UIControlStateNormal];
...
//    [[UserProfileManager sharedInstance] loadUserProfileInBackgroundWithBuddy:self.contactsSource saveToLoacal:YES completion:NULL];
    // hx头像昵称
    [[UserProfileManager shareInstance] loadUserProfileInBackgroundWithBuddy:self.contactsSource];
...
//        }
        cell.avatarView.image = [UIImage imageNamed:@"kefu13"];
        cell.titleLabel.text = kServiceNiceName;
    }
    else{
...
//        }
        // hx头像昵称
        UserCacheInfo *profileEntity = [UserProfileManager getById:model.buddy.username];
        if (profileEntity) {
            model.avatarURLPath = profileEntity.AvatarUrl;
            model.nickname = profileEntity.NickName == nil ? profileEntity.Id : profileEntity.NickName;
        }
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [self deleteFriendHandles:model.buddy.username];
        [self hideHud];
    }
}
...
//            NSString *firstLetter = [EaseChineseToPinyin pinyinFromChineseString:[[UserProfileManager sharedInstance] getNickNameWithUsername:buddy.username]];
            // hx头像昵称
            model.nickname = [UserProfileManager getNickById:buddy.username];
...
#pragma mark - 好友列表变化时 - 自己添加
-(void)registerNotifications
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didUpdateBuddiedList) name:@"didUpdateBuddiedList" object:nil];
}

- (void)didUpdateBuddiedList {
    [self reloadDataSource];
}
...
-(void)deleteFriendHandles:(NSString*)friendname{

    // 公钥加密
    NSString *encryWithPublicKeyStr = [RSAUtil encryptString:[NSString stringWithFormat:@"%@",User_ID] publicKey:kRSA_Public_key];
    
    [FKL_DataService requestURL:[NSString url_deleteFriendHandles:friendname] parameters:nil headerKey:@"sign" headerValue:encryWithPublicKeyStr withType:@"DELETE" format:@"JSON" complete:^(id result) {
