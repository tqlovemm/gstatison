...
//    ContactListViewController *_contactsVC;
    ...
    WomanMineController * _womanVC;
    XDSeekAndFlopController *_seekVC;
//    __weak CallViewController *_callController;
    XDPostsController *_postVC;
    XDSeekForGirlViewController *_seekForGirlVC;
...
//    [self didUnreadMessagesCountChanged];
    ...
    // 更新未读消息数量
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getUnreadCount) name:@"unreadOtherMessageCount" object:nil];
...
//        NSString *encryWithPublicKeyStr = [RSAUtil encryptString:[NSString stringWithFormat:@"%@",User_ID] publicKey:kRSA_Public_key];
        ...
        [FKL_DataService requestURL:[NSString url_judgeUserInfoWithUserId:user.thirteen_platform_number] parameters:nil withType:@"GET" format:@"JSON" complete:^(id result) {
            if ([result[@"code"] integerValue] == 201 && [[[EaseMob sharedInstance] chatManager] isLoggedIn]) {
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"我们发现你的信息不够完整，为了能更好的使用app,请先完善基本信息" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
                alert.tag = 102;
                [alert show];
            } else if ([result[@"code"] integerValue] == 200) { // 信息完整
...
- (void)setupTabBar {
    // 创建自定义tabbar
    XDTabBar *customTabBar = [[XDTabBar alloc] init];
    customTabBar.tabBarDelegate = self;
    customTabBar.backgroundColor = RGB(16, 16, 16);
    // 更换系统自带的tabbar
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (alertView.tag == 99) {
        ...
        if (buttonIndex != [alertView cancelButtonIndex]) {
            ...
            editVC.title = kEditDataTitle;
            [sqNav pushViewController:editVC animated:YES];
        }
    }
}
...
- (void)setupSubviews
{
    ...
    theme.navigationBarColor = NavigationBarColor;
    theme.navigationBarTintColor = NormalNavBtnColor;
    theme.orderBtnColor = NormalNavBtnColor;
    [[MMEmotionCentre defaultCentre] setTheme:theme];

//    self.tabBar.shadowImage = [UIImage imageWithColor:HEXCOLOR(0xdedede)];
    self.tabBar.shadowImage = [UIImage imageWithColor:HEXCOLOR(0xe9e9e9)];
    self.tabBar.backgroundImage = [[UIImage imageNamed:@"tabbarBkg"] stretchableImageWithLeftCapWidth:25 topCapHeight:2];

        // 消息
        _chatListVC = [[ConversationListController alloc]init];
        [self addOneChlildVc:_chatListVC title:@"消息" imageName:@"tabbar_mainframe" selectedImageName:@"tabbar_mainframeHL"];
        
        //觅约功能
        _seekVC = [[XDSeekAndFlopController alloc]init];
        [self addOneChlildVc:_seekVC title:@"约会" imageName:@"tabbar_seek" selectedImageName:@"tabbar_seekHL"];
        
        // 互撩广场
        XDActivitylistController *activityVC = [[XDActivitylistController alloc] init];
        [self addOneChlildVc:activityVC title:nil imageName:@"tabbar_plusButton" selectedImageName:@"tabbar_plusButtonHL"];
        
        // 帖子
        _postVC = [[XDPostsController alloc] init];
        [self addOneChlildVc:_postVC title:@"发现" imageName:@"tabbar_discover" selectedImageName:@"tabbar_discoverHL"];
        
        //个人中心
        _mineVC = [[MineViewController alloc]init];
        [self addOneChlildVc:_mineVC title:@"我的" imageName:@"tabbar_me_man" selectedImageName:@"tabbar_meHL_man"];
...
{
    [self setupTabBarItemChlildVc:childVc title:title imageName:imageName selectedImageName:selectedImageName];
    
    // 添加为tabbar控制器的子控制器
    XDNavigationController *nav = [[XDNavigationController alloc] initWithRootViewController:childVc];
    [self addChildViewController:nav];
}

- (void)setupTabBarItemChlildVc:(UIViewController *)childVc title:(NSString *)title imageName:(NSString *)imageName selectedImageName:(NSString *)selectedImageName {
    // 设置标题
    childVc.title = title;
    if ([childVc isKindOfClass:[XDActivitylistController class]]) {
...
//        childVc.tabBarItem.title = nil;
    ...
    selectedTextAttrs[NSForegroundColorAttributeName] = HEXCOLOR(0xCA9D56);
    [childVc.tabBarItem setTitleTextAttributes:selectedTextAttrs forState:UIControlStateSelected];
    
    // 设置图标
    UIImage *normalImg = [UIImage imageWithName:imageName];
    // 设置选中的图标
...
{
    [self jf_setupUnreadMessageCount];
}

// 统计未读消息数
-(void)jf_setupUnreadMessageCount {
...
//    NSInteger unreadCountApply = [[[ApplyViewController shareController] dataSource] count];
    // 申请与通知
...
//    UITabBarItem *tabBarItem0 = self.tabBar.items[0];
    UITabBarItem *tabBarItem0 = _chatListVC.tabBarItem;
    
    XDUnreadCount *unReadCountModel = [[XDUnreadCount alloc] init];
    
    if ((unreadCountApply + unreadCount + unReadCountModel.system_total_count) > 0) {
        tabBarItem0.badgeValue = [NSString stringWithFormat:@"%i",(int)((unreadCountApply + unreadCount + unReadCountModel.system_total_count))];
    }
    else {
        tabBarItem0.badgeValue = nil;
    }
    
    UIApplication *application = [UIApplication sharedApplication];
    [application setApplicationIconBadgeNumber:unreadCount + unreadCountApply + unReadCountModel.msg_total];
...
{
    if ([userNames containsObject:kServiceName]) { // 客服会话消息不移除
        NSMutableArray *names = [NSMutableArray arrayWithArray:userNames];
        [names removeObject:kServiceName];
        userNames = names;
...
//    [_contactsVC reloadDataSource];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"didUpdateBuddiedList" object:nil];
}
...
//    while (resultVC.presentedViewController) { // 添加此段代码包含模态控制器
...
{
    // 2.获得未读数
    [FKL_DataService encryption_requestURL:[NSString url_getAllNewUnreaadMessageCount] parameters:nil withType:@"GET" complete:^(id result) {
    
        if ([result[@"code"] integerValue] == 200) {
            XDUnreadCount *unreadCount = [XDUnreadCount objectWithKeyValues:result[@"data"]];
            
            // 帖子消息未读数
//            if (unreadCount.thread_count == 0) {
//                _postVC.tabBarItem.badgeValue = nil;
//            } else {
//                _postVC.tabBarItem.badgeValue = [NSString stringWithFormat:@"%d", unreadCount.thread_count];
//            }
//
//            [_postVC reloadUnreadMessageCount];
            ...
            if ((unreadCountApply + unreadMessageCount + unreadCount.system_total_count) > 0) {
                //tabBarItem0.badgeValue = [NSString stringWithFormat:@"%ld", unreadCountApply + unreadMessageCount + unreadCount.system_total_count];
                tabBarItem0.badgeValue = [NSString stringWithFormat:@"%ld", unreadMessageCount + unreadCount.system_total_count];
            ...
            [_chatListVC refreshDataSource];
            // 在图标上显示所有的未读数
            [[UIApplication sharedApplication] setApplicationIconBadgeNumber:unreadMessageCount + unreadCountApply + unreadCount.msg_total];
        } else {
            [self.view makeToast:result[@"message"] duration:2.0 position:CSToastPositionCenter];
        }
    } faileComplete:^(NSURLSessionDataTask *task, NSError *error) {
